...
CREATE TABLE producto(
    ...
    UNIQUE KEY uq_producto_nombre (nombre) -- crea llave unica en el nombre para que el nombre no se repita
);

-- 2) Función: devuelve id por nombre (solo si está activo) o -1 si no existe
DROP FUNCTION IF EXISTS fn_producto_id_por_nombre;
DELIMITER $$ -- no terminar funcion hasta que no encuentre el $$
